...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    # YOUR CODE HERE
    #distances = torch.exp((-1./sigma) * (torch.matmul(x_1 - x_2, (x_1 - x_2).T)))
    #(np.sum(X**2, axis=1))[:, np.newaxis] + np.sum(self.X_train**2, axis=1).T - 2*X.dot(self.X_train.T)
    
    #distances = torch.exp((-1./sigma) * ((torch.sum(x_1**2, dim=1))[:, :, None] + torch.sum(x_2**2, dim=1).T[None, :, :] - 2*matmul(x_1, x_2.T)))
    
    
    x_1_norm = torch.sum(x_1 ** 2, axis=-1)
    x_2_norm = torch.sum(x_2 ** 2, axis=-1)
    distances =torch.exp(-0.5/(sigma**2) *(x_1_norm[:,None] + x_2_norm[None,:] - 2. * np.dot(x_1, x_2.T)))
    
    #distances = torch.exp((-0.5/sigma**2) * torch.pow(torch.norm((x_1[:, :, None] - x_2.T[None, :, :]), p='fro', dim=1), 2))
    # YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    # YOUR CODE HERE
    #torch.mean(torch.clamp((1 - torch.matmul(scores, labels)), 0))
